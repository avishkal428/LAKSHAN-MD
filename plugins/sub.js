const { cmd } = require("../command"); // Path එක නිවැරදි කරන ලදී
...
// Helper to send Image Card safely
async function sendPosterCard(conn, from, posterUrl, captionText, mek, reply) {
        ...
        try {
            ...
            if (session.step === "WAITING_MOVIE_SELECTION") {
                ...
                if (isTVSeries) {
                    ...
                    await sendPosterCard(conn, from, details.poster, epCard, mek, reply);
                    return;
                }
                ...
                await sendPosterCard(conn, from, details.poster, detailsCard, mek, reply);
                return;
            }
